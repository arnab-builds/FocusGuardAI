...
function EmployeeAnalyticsDialog({
    ...
    useEffect(() => {
        ...
        async function loadHistory() {
            ...
            try {
                const data = await getEmployeeBrowsingHistory(
                    employee.id,
                    {
                        date: historyDate,
                        page: historyPage,
        ...
        loadHistory();
    }, [open, employee?.id, historyDate, historyPage]);

    if (!employee) return null;

    const productivity = getRoundedProductivity(employee);
    const productiveTime = formatDuration(
        employee.productive_time ||
            employee.analytics?.productive_time ||
            "0:00:00"
    );
    const nonProductiveTime =
        employee.non_productive_time ||
        employee.unproductive_time ||
        employee.analytics?.non_productive_time ||
        employee.analytics?.unproductive_time ||
        "0:00:00";
                    ...
                    <div className="grid grid-cols-2 gap-5">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
                            ...
                            <h3 className="mt-2 text-lg font-bold">
                                ...
                                {formatDuration(nonProductiveTime)}
